...
const Index = () => {
  ...
  const handleSend = async (text: string) => {
    ...
    const API_URL = import.meta.env.VITE_API_URL || "https://legal-assistant-rag.onrender.com";

    // Wake Render up before sending the real request
    const wakeBackend = async (): Promise<void> => {
      try {
        await fetch(`${API_URL}/health`, { method: "GET", signal: AbortSignal.timeout(15000) });
      } catch {
        // ignore — we just want to trigger the cold start
      }
    };

    try {
      // Wake the instance first, then wait a moment
      await wakeBackend();
      ...
      const sources = Array.isArray(data.source_chunks)
        ? data.source_chunks.map((chunk: any) => ({
          id: chunk.chunk_id,
